...
<script type="text/javascript">
	function check(pages){
		document.getElementById("hid").value=pages;
...
<body>
<form action="login" method="post" id="fim">
<input type="hidden" name="currentPage" id="hid"/>
...
<c:if test="${pagebean.currentPage>1}">
	<a href="javascript:check(${1})">首页</a>
	<a href="javascript:check(${pagebean.currentPage-1})">上一页</a>
</c:if>

<c:forEach var="num" begin="1" end="${pagebean.totalPage}">
	<a href="javascript:check(${num})">${num}</a>
</c:forEach>

<c:if test="${pagebean.currentPage<pagebean.totalPage}">
	<a href="javascript:check(${pagebean.currentPage+1})">下一页</a>
	<a href="javascript:check(${pagebean.totalPage})">末页</a>
</c:if>
